...
def _fundamento_texto_extendido(na, k, ph, pam, vasopresor_alto, lactato_desc,
    ...
    return [
        ...
        "• Citrato trisódico 4% aporta Na: 3 x 136 = 408 mmol/L de Na → balance de sodio importante.",
...
BIBLIO: List[Ref] = [
    ...
    {"id": "hd_adequacy_2021", "yr": "2021", "title": "Hemodialysis Adequacy: 2020 Update",
     ...
     "tags": "hd,ktv,adecuacion", "blurb": "Kt/V ≥1.2 por sesión (3x/semana); Kt/V 1.4 objetivo."},
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "HNF" in ac_sel_presc:
        ...
        hnf_dose_presc = st.number_input(
            ...
            key="presc_hnf_dose",
            help=f"Estimado: 5 UI/kg/hr x {peso:.0f} kg = {hnf_default} UI/hr. Ajustar a aPTT 45–70 s.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if tpe_modo == "⚗️ Cálculo de sesión":
        ...
        tpe_r1.metric("Volumen plasmático estimado (VPE)", f"{result_tpe['EPV']:.0f} mL",
                      help="Fórmula: 65 x peso x (1 − Hto)")
        tpe_r2.metric("Volumen a intercambiar", f"{result_tpe['vol_ex']:.0f} mL",
                      help=f"{tpe_nex:.1f} recambios x VPE")
...
Concentraciones mayores (20%, 25%) deben diluirse antes de usarse como reemplazo.
            ...
            if "20%" in alb_pct_vial:
                pct_vial = 20
                g_por_vial = 10.0  # 50mL x 20% = 10g
            elif "25%" in alb_pct_vial:
                pct_vial = 25
                g_por_vial = 12.5  # 50mL x 25% = 12.5g
            else:
                pct_vial = 5
                g_por_vial = 2.5   # 50mL x 5% = 2.5g
...
**Para toda la sesión:** preparar {n_bolsas_total} bolsas x {viales_por_bolsa} viales = **{viales_total} viales en total**
                """)
            ...
            vr2.metric("Bolsas necesarias", f"{math.ceil(vol_intercambio/alb_bolsa_ml)} x {alb_bolsa_str}")
...
2. **Aporta:** ADAMTS13 funcional contenido en el PFC (plasma fresco congelado)
                ...
                "frecuencia": "Diario hasta remisión (plaquetas >150,000 x 2 días consecutivos)",
...
- Sin beneficio demostrado si el paciente ya es diálisis-dependiente sin hemorragia pulmonar
                ...
                "frecuencia": "Diario x 14 días o hasta anti-MBG negativo",
...
- Complementaria a inmunosupresión (ciclofosfamida + prednisona)
                ...
                "frecuencia": "Cada día o cada tercer día x 7 sesiones en 2 semanas",
...
- Criterio de respuesta: disminución de proteinuria >50% del basal
                ...
                "frecuencia": "Diario x 5–10 sesiones iniciales; luego espaciar según respuesta",
...
- Respuesta evaluada por DSA en MFI post-sesión
                """,
                "reemplazo": "✅ **Albúmina 5%**",
                "frecuencia": "Cada día o cada tercer día x 5–10 sesiones",
...
- No modifica la historia natural — requiere inmunosupresión de mantenimiento
                """,
                "reemplazo": "✅ **Albúmina 5%**",
                "frecuencia": "Cada día o cada tercer día x 5–6 sesiones (crisis) o pre-timectomía",
...
- Inicio temprano (<2 semanas del inicio) → mejor respuesta
                ...
                "frecuencia": "Cada día o cada tercer día x 4–6 sesiones",
...
- TPE como puente hasta que la quimioterapia surta efecto
                """,
                "reemplazo": "✅ **Albúmina 5%**",
                "frecuencia": "Diario x 3–5 sesiones o hasta remisión sintomática",
...
- Combinada con inmunosupresión (rituximab) y tratamiento del VHC si aplica
                ...
                "frecuencia": "Cada día o cada tercer día x 3–8 sesiones según severidad",
    ...
    # ── MAGNESIO ───────────────────────────────────────────────────────────────
        ...
        with mg2:
            st.metric("Mg en mmol/L", f"{mg_act * 0.4113:.2f}",
                      help="Conversión: mg/dL x 0.4113 = mmol/L")
...
| >2,200 | Rango nefrótico | A3 |
            """)
            st.caption("**Conversión rápida:** mg/dL x 10 = mg/L | Resultado en mg/g (equivalente a mg/mmol x 8.84)")
...
| **Hepatorrenal** | <1% | <35% | Como prerrenal |
        ...
        # ── OSMOLALIDAD ───────────────────────────────────────────────────────
            ...
            st.caption("Fórmula: 2xNa + Glucosa/18 + BUN/2.8 (+Etanol/4.6). Normal: 280–295 mOsm/kg. Gap normal: <10.")
        ...
        # ── CORRECCIÓN DE CALCIO ──────────────────────────────────────────────
            ...
            st.info("Si albumina baja, el calcio total subestima el Ca ionizado real. "
                    "Fórmula: Ca_corr = Ca_total + 0.8 x (Albúmina_normal – Albúmina_paciente)")
...
- **HD crónica:** 100–200 mg IV **post-sesión** x 10 sesiones (1,000–2,000 mg total)
- **Prediálisis / PD:** 200 mg IV en 100 mL NaCl 0.9% en 30 min, c/semana x 5
...
- Menor uso por mayor riesgo de reacciones (anafilaxia 0.6%)
        ...
        if "nuevo" in aee_modo:
            ...
            if "alfa" in aee_nuevo_tipo and "Darbe" not in aee_nuevo_tipo:
                ...
                frec = "3 x semana IV post-sesión" if en_hd else \
                       "3 x semana SC (o 1x semana esquema simplificado)"
...
| **Meta Hgb** | **10 – 11.5 g/dL** (no pasar de 13) |
            ...
            # ── Epoetina beta ──────────────────────────────────────────────────
                ...
                dosis_low  = 50 * peso_aee
                dosis_high = 100 * peso_aee
                frec_beta = "3 x semana IV post-sesión" if en_hd else \
                            "1–3 x semana SC o 1x semana (mantenimiento SC)"
...
| **Ventaja vs epoetina alfa** | Se puede dar **1x semana SC** en mantenimiento (menor frecuencia) |
| **Primera evaluación** | **4 semanas** |
...
|---------|---------------------------|---------------|-----------|-----|
| **Semanal** | **{dosis_sem:.1f} mcg** | {vial_darb_sem} mcg | 1 x semana | {via_darb} |
| **Quincenal** | **{dosis_2sem:.1f} mcg** | {vial_darb_2s} mcg | 1 x 2 semanas | {via_darb} |
...
| | |
|---|---|
| **Dosis** | **{dosis_mircera:.1f} mcg** ({peso_aee:.0f} kg x 0.6 mcg/kg) |
...
> **Ventaja clave:** funcionan incluso en presencia de inflamación (donde los AEE clásicos pierden eficacia por hepcidina elevada)
            ...
            hif_data = {
                "Roxadustat (Evrenzo®)": {
                    ...
                    "dosis_inicio": "70–100 mg 3x/semana VO",
...
|--|--------------|---------|
| **Vía** | SC / IV | **Oral** |
| **Frecuencia** | 1–3x/semana | Diario o 3x/semana |
        ...
        # ── MODO 2: AJUSTE ────────────────────────────────────────────────────
            ...
            with adj1:
                ...
                if "Epoetina" in aee_actual and "Metoxi" not in aee_actual:
                    ...
                    freq_sel  = st.selectbox("Frecuencia", ["3x/semana","2x/semana","c/semana"], key="aee_freq")
                    unidad    = "UI"
                    # Weekly dose
                    freq_factor = {"3x/semana": 3, "2x/semana": 2, "c/semana": 1}.get(freq_sel, 1)
...
#### Calculadora de ajuste empírico de tacrolimus
        ...
        st.caption("Monitorear niveles C0 (valle, antes de la dosis): diario x 1 semana → semanal x 1 mes → mensual. "
...
**Si no responde:** biopsia de confirmación + timoglobulina (rechazo celular ≥IB) o IVIG + Rituximab (rechazo humoral)
    ...
    # ── PROTOCOLO DE RECHAZO ──────────────────────────────────────────────────
        ...
        protocolos = {
            "Rechazo Celular Agudo (ACR) Banff IA — tubulitis moderada": {
                "primera_linea": "Pulsos de metilprednisolona 500 mg IV x 3 días",
                "segunda_linea": "Timoglobulina 1.5 mg/kg/día x 7–10 días si no responde",
                "ajuste_is": "Optimizar niveles de tacrolimus (C0 10–15 ng/mL fase aguda)",
                "pronostico": "80–90% respuesta a esteroides",
                "seguimiento": "Creatinina diaria x 1 semana · Biopsia de control si no mejora en 5–7 días",
            },
            "Rechazo Celular Agudo (ACR) Banff IB — tubulitis severa": {
                "primera_linea": "Pulsos de metilprednisolona 500 mg IV x 3 días",
                "segunda_linea": "Timoglobulina 1.5 mg/kg/día x 7–14 días (alta probabilidad de necesitar)",
            ...
            "Rechazo Celular Agudo (ACR) Banff IIA/IIB — arteritis": {
                "primera_linea": "Timoglobulina 1.5 mg/kg/día x 7–14 días (primera línea directa)",
            ...
            "Rechazo Humoral Agudo (AMR) — DSA + C4d + lesión microvascular": {
                ...
                "segunda_linea": "Rituximab 375 mg/m² x 1–4 dosis (si no responde o recurrente)",
...
|--------|-------|---------------------|
| **Ciclofosfamida VO** | 2–2.5 mg/kg/día x 8–12 semanas | 2B |
| **Tacrolimus** | 0.05–0.1 mg/kg/día c/12h (nivel 5–10 ng/mL) | 2C |
| **Rituximab** | 375 mg/m² x 1–4 dosis | 2C |
...
|--------|-------|-----------|
| **Tacrolimus** | 0.05–0.1 mg/kg/día c/12h x mín. 6–12 meses | 2C |
| **Ciclosporina** | 4–6 mg/kg/día c/12h | 2C |
| **Ciclofosfamida IV** | 500–1000 mg/m² mensual x 6 meses | 2C |
...
|-----------|-----------|------|
| Semanal x 4 semanas | Proteinuria (tira reactiva o RPCU) | Remisión: <300 mg/día |
...
**Prednisona:** 1 mg/kg/día (máx 80 mg) x mínimo **16 semanas** antes de clasificar resistencia.

**Si resistente a esteroides:**
| Opción | Dosis KDIGO 2021 |
|--------|-----------------|
| **Tacrolimus (CNI preferido)** | 0.05–0.1 mg/kg/día c/12h (nivel 5–10 ng/mL) x ≥12 meses | 
| Ciclosporina | 3–5 mg/kg/día c/12h x ≥12 meses |
...
| **② Rituximab** | 375 mg/m² x 4 semanas **o** 1g x 2 (día 1 y 15) | 2C |
...
| Opción | Descripción | Evidencia |
|--------|-------------|-----------|
| **① Rituximab (preferido)** | 375 mg/m² x 4 semanas **o** 1g x 2 | 2B |
| **② CNI → + Rituximab** | CNI x 6 meses → agregar rituximab si no hay respuesta (excepto si PLA2R desapareció) | 2C |
| **③ Ciclofosfamida + esteroides** | Esquema Ponticelli alternado x 6 meses | 2B |

#### 🚨 Riesgo MUY ALTO / SN amenazante de vida:
| Opción | Descripción | Evidencia |
|--------|-------------|-----------|
| **① Ciclofosfamida + esteroides** (1ª línea) | Esquema Ponticelli modificado x 6 meses | 1B |
...
- Meses 1, 3, 5: Metilprednisolona 1g IV x 3 días → Prednisona 0.5 mg/kg/día x 27 días
- Meses 2, 4, 6: Ciclofosfamida VO 2.5 mg/kg/día x 30 días
...
| **Budesonida MR (Nefecon®)** 16 mg/día VO x 9 meses | Si proteinuria persistente ≥1 g/día con RASi optimizado | 2B |
...
| Agente | Dosis | Evidencia |
|--------|-------|-----------|
| **Rituximab (1ª línea)** | 375 mg/m² x 4 semanas O 1g x 2 (d1, d15) | 1B |
| **Ciclofosfamida IV** | 15 mg/kg c/2 semanas x 3, luego c/3 semanas x 3–6 | 1B |
| **Glucocorticoides** | Metilprednisolona 1–3 g IV x 3 días → prednisona 1 mg/kg/día (reducción en 5–6 meses) | — |
        ...
        with ln2:
            tratamientos_ln = {
                "III": {
                    ...
                    "alternativa": "Ciclofosfamida IV bajas dosis (Euro-Lupus) 500 mg c/2 semanas x 6",
                ...
                "IV": {
                    ...
                    "alternativa": "Ciclofosfamida IV altas dosis (NIH) 0.5–1 g/m² mensual x 6",
...
|-----------|---------|
| **Plasmaféresis** | 4L/sesión c/día x 14 días o hasta anti-MBG negativo |
| **Ciclofosfamida VO** | 2–3 mg/kg/día x 3 meses (ajustar por edad y función renal) |
| **Prednisona** | 1 mg/kg/día (máx 60–80 mg) → reducción progresiva en 6 meses |
| **Pulsos MP** | Metilprednisolona 500–1000 mg IV x 3 días si afección pulmonar severa |
...
| **Endocarditis** | Antibióticos según cultivo x 6 semanas + reparación valvular si aplica |
| **HIV** | TARV optimizado — NF puede mejorar con supresión viral |
| **Lupus** | Ver módulo Nefritis Lúpica |
| **Crioglobulinemia tipo II/III** | Rituximab 375 mg/m² x 4 sem. Si VHC positivo: DAA + rituximab |
...
|-----------|------------|
| **Anti-factor H** | Plasmaféresis x 5–7 sesiones + rituximab 375 mg/m² x 4 sem (urgente) |
...
- Monitorear glucemia c/30 min x 2h (riesgo de hipoglucemia)
            """)
...
| **Ciclosilicato de Zirconio (ZS-9)** | 10g VO c/8h x 3 dosis | 1h |
...
| Tac C0 <6 ng/mL x 2 mediciones | Ajustar dosis + investigar cumplimiento |
...
MMF: mantener ≥1 g/día x 12 meses, luego reducir según tolerancia
```
...
| Anual x indefinido | Cr, PA, proteinuria | Detección precoz de ERC de novo |
        ...
        with st.expander("🦠 Infectología (si TB latente, HIV, infecciones complejas)"):
            ...
            if ec_infecto_ind:
                ec_tb_plan = st.selectbox("Plan para TB latente", [
                    ...
                    "Isoniazida 300 mg/día x 9 meses — en curso",
                    "Isoniazida 300 mg/día x 9 meses — completada",
                    "Rifampicina x 4 meses — en curso","Rifampicina x 4 meses — completada",
...
| **Glucosa** | c/6h x 48h | NODAT: insulina de corrección si >180 mg/dL |
...
| **Fluconazol** (hongos) | Día 1 | 100–200 mg/día x 1–3 meses |
| **Nistatina** (oral) | Desde día 0 | 500,000 UI c/8h x 2 semanas |

#### Prednisona post-Tx — Selecciona el protocolo

> Existen varios esquemas validados. La evidencia más reciente favorece el **taper rápido o la minimización** en pacientes de bajo riesgo inmunológico para reducir complicaciones metabólicas (NODAT, hiperlipidemia, osteoporosis) sin aumentar el riesgo de rechazo.

| Protocolo | Velocidad | Mejor para | Evidencia |
|-----------|----------|-----------|----------|
| **INNSZ / Rápido** | Mantenimiento en 4–6 semanas | Bajo riesgo inmunológico | Alta — práctica actual Mexico |
| **KDIGO estándar** | Mantenimiento en 3–6 meses | Riesgo moderado-alto | KDIGO 2009 |
| **Minimización temprana** | Retiro a mes 3–6 | Bajo riesgo, sin episodios de rechazo | ELITE-Symphony, FREEDOM |
| **Esteroides indefinidos** | 5 mg/día de por vida | Alto riesgo, LES, rechazo previo | Práctica clásica |

```
⭐ PROTOCOLO INNSZ / RÁPIDO — Recomendado en bajo riesgo:
  Día 0 (QX):   Metilprednisolona 500 mg IV (intraoperatorio)
  Día 1:        Prednisona 30 mg/día VO
  Días 2–7:     20 mg/día
  Semana 2–3:   15 mg/día
  Semana 4:     10 mg/día
  Mes 2 en adelante: 5 mg/día (mantenimiento)

  ✅ Ventaja: menor NODAT, menos Cushingoide, misma tasa de rechazo en bajo riesgo
  ✅ Respaldado por práctica de centros de referencia en México (INNSZ, CMN Siglo XXI)

📋 PROTOCOLO KDIGO ESTÁNDAR — Moderado a alto riesgo:
  ...
  ⚠️ Mayor exposición acumulada a esteroides → más NODAT, HTA, hiperlipidemia

🚀 MINIMIZACIÓN TEMPRANA — Bajo riesgo + función estable:
  Igual que INNSZ rápido hasta mes 1
  Mes 3–6: reducir a 2.5 mg/día
  Mes 6–12: RETIRO COMPLETO si sin episodios de rechazo
  (Solo en pacientes seleccionados con tacrolimus C0 en rango + sin proteinuria)
  
  ⚠️ No recomendado si: retrasplante, PRA >30%, rechazo previo, LES/vasculitis
```
        """)
...
|-----------|-----------|------------------|
| **Diuresis** | Cada hora | <30 mL/h x 2h → ver algoritmo |
...
| Criterio | Umbral |
|---------|--------|
| Glucosa en ayuno | ≥126 mg/dL x 2 mediciones |
...
|-----------|------|----------------------|
| LDL-C | **<100 mg/dL** (idealmente <70 en alto riesgo CV) | Cada 3–6 meses x 1 año, luego anual |
        ...
        if faltantes:
            ...
            with rx_tab1:
                ...
                rx_body = st.text_area("Indicaciones *", height=180, key="rx_body",
                    placeholder="1. Tacrolimus (Prograf®) 1 mg c/12h VO x indefinido\n"
...
- Monitoreo de CMV DNAemia: c/1–2 semanas x primeros 3 meses
...
- BKPyV DNAemia en plasma (PCR cuantitativa): **mensual x 9 meses → c/3 meses hasta 2 años**
...
| **>1,000 copies/mL x 3 semanas** o **>10,000 copies/mL** | **BKPyVAN probable / presuntiva** | **Reducir IS** |
...
```
BKPyV DNAemia >1,000 c/mL x 3 sem o >10,000 c/mL
    │
    └─ Reducir MMF 25–50%
              │
    ┌─────────┴────────────┐
...
- Si DNAemia negativa x 3 meses: IS puede reanudarse gradualmente
```
...
|-----------|------------|
| **Leve-moderada** (PaO2 >70 mmHg) | TMP-SMX 15–20 mg/kg/día (componente TMP) VO dividido c/8h x **21 días** |
| **Grave** (PaO2 <70 mmHg) | TMP-SMX IV misma dosis x 21 días + **prednisona 40 mg c/12h x 5 días → taper** |
...
|-----------------|-------------|
| **D+/R− (EBV)** | Mayor riesgo de PTLD (10–15x vs D+/R+) |
...
**Screening de EBV DNA (en plasma):** Mensual x 3–6 meses, luego c/3 meses hasta 1 año (especialmente D+/R−).
...
EBV DNAemia elevada (sin masa)
    ...
    └─ Si masa / síntomas → Biopsia (PET-TC para estadificación)
              │
              ├─ CD20+: Rituximab 375 mg/m² x 4 ciclos
              │    │
...
| **Pielonefritis del injerto** | Fiebre + dolor en injerto + urocultivo+ | ATB IV x 14 días + imágenes del injerto |
...
#### Profilaxis
- **TMP-SMX** (ya cubre PCP + IVU): 1 tab simple c/24h x 3–6 meses
...
|-------------|-----------------|-----------|
| **IVIG** (1ª línea) | **0.4 g/kg/día x 5 días** (total: 2 g/kg) IV | Series de casos — evidencia moderada |
...
| **Sintomática** (cistitis) | Fluconazol 200 mg/día VO x 14 días |
| **Pielonefritis** del injerto por Candida | Fluconazol 400 mg/día x 14 días o equinocandina IV x 14 días |
| **C. glabrata** (fluconazol resistente) | Anidulafungina o caspofungina IV |

**Profilaxis antifúngica:** Fluconazol 100–200 mg/día VO x 1–3 meses (especialmente post-timoglobulina o en centros de alto riesgo).

#### Candidemia — protocolo urgente
1. Hemocultivos x 2 + fungemias
...
|-----------|---------|-------|---------|
| **1ª línea** | **Voriconazol VO/IV** | 6 mg/kg c/12h x 2 dosis → 4 mg/kg c/12h IV, luego 200 mg c/12h VO | Mínimo 6–12 semanas |
| **Alternativa** | Isavuconazol | 200 mg c/8h x 6 dosis → 200 mg c/24h | Igual que voriconazol |
...
**Máximo 6 mismatches posibles** (2 por locus x 3 loci: A + B + DR).
...
- Desensibilización pre-trasplante recomendada:
  ...
  → Plasmaféresis si DSA fuertes (x5 sesiones pre-trasplante)
...
- Desensibilización agresiva pre-trasplante:
  → IVIG 2 g/kg + Rituximab x múltiples ciclos
  → Plasmaféresis intensiva (x 10–15 sesiones)
...
|------|-------------|---------|
| **1** | IVIG 2 g/kg IV (1 g/kg x 2 días) | Bloqueo Fc + eliminación de anticuerpos |
| **2** | Rituximab 375 mg/m² x 1 dosis | Depleción de células B productoras de anticuerpos |
| **3** | Repetir IVIG en 21–30 días si DSA persisten | Reducción sostenida |
| **4** | Plasmaféresis x 5–10 sesiones si DSA fuertes (MFI >5,000) | Eliminación directa de anticuerpos |
...
- **IMC >30:** diuresis post-clampeo es predictora; furosemida intraoperatoria
    ...
    _MEDS_DB = [
        ...
        # ── INMUNOSUPRESORES ───────────────────────────────────────────────────
        {"g":"Tacrolimus","b":["Prograf","Advagraf","Envarsus"],"p":["cápsulas 0.5 mg","cápsulas 1 mg","cápsulas 5 mg","cápsulas LP 0.5 mg","cápsulas LP 1 mg","cápsulas LP 5 mg"],"d":"0.1 mg/kg/día c/12h VO (Prograf) o c/24h (Advagraf)","cat":"CNI","inter":["Azoles","Diltiazem","Verapamilo","AINEs","Rifampicina","Eritromicina","Claritromicina"],"nota":"⚠️ Monitorear niveles C0. CYP3A4 — múltiples interacciones"},
