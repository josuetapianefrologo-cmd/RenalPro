...
with tab_main:
    ...
    # ── Ajuste de flujos cuando citrato está activo ──────────────────────────
    ac_activa_main = st.session_state.get("anticoagulacion_tipo", "—")
    v_cit_main = float(st.session_state.get("rca_citrato_ml_h", 0))
    if ac_activa_main == "RCA" and v_cit_main > 0:
        st.markdown("#### 🧪 Flujos ajustados por citrato (RCA activo)")
        qr_pre_sol = max(0.0, qr_pre - v_cit_main)
        pred_ef_total = qr_pre_sol + v_cit_main   # = qr_pre original (o v_cit si cubre todo)
        ff_adj_val = (qr_post + uf) / max(qp_h + qr_pre_sol + v_cit_main, 1e-9)
        ff_adj_pct = ff_adj_val * 100
        cadj1, cadj2, cadj3, cadj4 = st.columns(4)
        cadj1.metric("V_citrato PRE (mL/hr)", f"{v_cit_main:.0f}",
                     help="Citrato va PRE-filtro → actúa como predilución")
        cadj2.metric("Qr PRE solución ajustado (mL/hr)", f"{qr_pre_sol:.0f}",
                     delta=f"{qr_pre_sol - qr_pre:+.0f} vs original",
                     help=f"Qr_pre {qr_pre:.0f} − V_cit {v_cit_main:.0f}")
        cadj3.metric("Predilución efectiva total (mL/hr)", f"{pred_ef_total:.0f}",
                     help="Solución PRE + citrato = predilución total pre-filtro")
        cadj4.metric("FF efectiva (con citrato)", f"{ff_adj_pct:.1f}%",
                     delta="✅ OK" if ff_adj_pct <= 25 else "⚠️ ALTA")
        if qr_pre_sol <= 0:
            st.info(f"💡 El citrato ({v_cit_main:.0f} mL/hr) cubre toda la predilución prevista. "
                    f"**Qr_pre de solución = 0 mL/hr** en la máquina. "
                    f"FF efectiva ≈ {ff_adj_pct:.1f}%")
        else:
            st.info(f"💡 Configura en la máquina → **Qr_pre solución: {qr_pre_sol:.0f} mL/hr** "
                    f"(citrato aporta {v_cit_main:.0f} mL/hr adicionales de predilución). "
                    f"FF efectiva ≈ {ff_adj_pct:.1f}%")
