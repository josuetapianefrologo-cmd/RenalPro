# app.py — TRRC360 by Dr. Tapia (v1.19.8)
...
def filtrar_refs_por_contexto(escenarios_sel: List[str], anticoag_tipo: str) -> List[Ref]:
    ...
    if any(x in e for x in ["intoxicación","intoxicacion","sobredosis"]): tags_req.update({"intoxicacion"})
    if any(x in e for x in ["neurocrítico","neurocritico","tce"]): tags_req.update({"neuro","osmolaridad"})
...
with tab_anticoag:
    ...
    if ac == "Heparina no fraccionada (HNF)":
        ...
        citrato_ml_h = (st.session_state.get("sb_qb",200) * 60.0 * objetivo_mmol_L_sangre / 1000.0) / max(citrato_mmol_por_ml, 1e-6)
        citrato_mmol_h = st.session_state.get("sb_qb",200) * 60.0 * (objetivo_mmol_L_sangre / 1000.0)
...
with tab_trends:
    ...
    # Captura T1-T2-T3 por analito en cuadrícula
    ...
    # Tabla compacta con barras en celda
    ...
    # === Selector visual horizontal (Altair) ===
    st.markdown("### Selector visual horizontal (T1–T3 por analito)")
    df_long = (
        df.reset_index()
          .rename(columns={"index":"Tiempo"})
          .melt(id_vars="Tiempo", var_name="Analito", value_name="Valor")
    )
    # Para que salgan todos los analitos en orden definido
    df_long["Analito"] = pd.Categorical(df_long["Analito"], categories=list(valores.keys()), ordered=True)
    chart = (
        alt.Chart(df_long)
        .mark_bar()
        .encode(
            y=alt.Y("Analito:N", sort=list(valores.keys()), title=None),
            x=alt.X("Valor:Q", title=None),
            color=alt.Color("Tiempo:N", legend=alt.Legend(orient="bottom")),
            tooltip=["Analito","Tiempo","Valor"]
        )
        .properties(height=28*len(valores), width=900)
...
with tab_fund:
    ...
    # LaTeX de fórmulas base (cadenas en una sola línea, correctamente cerradas)
    ...
    c1,c2,c3 = st.columns(3)
    with c1:
        st.write(f"**Qb** = {int(qb_v)}")
        st.write(f"**Hto** = {hto_v:.2f}")
        st.write(f"**Qp** = {int(qp_v)}")
    with c2:
        st.write(f"**Qp·h** = {int(qp_h_v)}")
        st.write(f"**Qe** = {int(qe_v)}")
        st.write(f"**UF** = {int(uf_v)}")
    with c3:
        st.write(f"**Qr_pre** = {int(qr_pre_v)}")
        st.write(f"**Qr_post** = {int(qr_post_v)}")
        st.write(f"**Qd** = {int(qd_v)}")
        st.write(f"**FF** ≈ {ff_txt_v}")
