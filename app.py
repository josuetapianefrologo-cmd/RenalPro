...
FILTROS: Dict[str, Filtro] = {
    "Oxiris (AN69-ST; adsorción alta)": Filtro(
        "Oxiris (AN69-ST; adsorción alta)", ["adsorción", "CVVHDF", "sepsis"], 1.5,
        "Adsorción de mediadores. Beneficio en desenlaces duros aún incierto."),
    "HCO 1100 (alta cut-off)": Filtro(
        "HCO 1100 (alta cut-off)", ["HCO", "CVVH", "mioglobina"], 1.1,
        "Alta permeabilidad para mioglobina. Vigilar albúmina (Alb <2.5: reconsiderar)."),
    "HCO 730 (alta cut-off)": Filtro(
        "HCO 730 (alta cut-off)", ["HCO", "CVVH", "mioglobina"], 0.7,
        "Similar a HCO 1100, menor área. Mismas precauciones de albúmina."),
    "Convectivo estándar (1.3 m²)": Filtro(
        "Convectivo estándar (1.3 m²)", ["convectivo", "CVVH", "CVVHDF"], 1.3,
        "Uso general; alternativa cuando no hay HCO/adsorción."),
    "Difusivo estándar (2.1 m²)": Filtro(
        "Difusivo estándar (2.1 m²)", ["difusivo", "CVVHD", "CVVHDF"], 2.1,
        "Útil si se prioriza depuración difusiva (urea/K)."),
}
...
(tab_main, tab_citrato, tab_na_trrc, tab_hd, tab_tpe,
 ...
 tab_enfermeria, tab_rx, tab_refs) = st.tabs([
    "🫀 Prescripción",
    "🧪 Citrato RCA",
    "🧂 Sodio TRRC",
    "💉 Predicción HD",
...
with tab_main:
    ...
    # Semáforo FF
    st.markdown("#### Estado del filtro (Fracción de Filtración)")
    ff_pct = ff * 100 if ff is not None else 0.0
    if ff_pct <= 20:
        st.success(f"✅ FF ≈ {ff_pct:.1f}% — Óptima. Mantener configuración.")
    elif ff_pct <= 25:
        st.warning(f"🟡 FF ≈ {ff_pct:.1f}% — Límite. Considerar ↑ predilución o ↑ Qb.")
    else:
        st.error(f"🔴 FF ≈ {ff_pct:.1f}% — ALTO RIESGO de coagulación. Reducir Qr_post o UF, ↑ predilución.")
...
with tab_citrato:
    ...
    with rc3:
        cit_qeff = st.number_input("Efluente total (mL/hr)", 0, 6000,
                                   int(dosis_mlkg * peso) if peso > 0 else 2000,
                                   100, key="cit_qeff")
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 10: ORDEN DE ENFERMERÍA
# ══════════════════════════════════════════════════════════════════════════════
with tab_enfermeria:
    st.subheader("🏥 Tarjeta de Orden de Enfermería — TRRC")
    st.caption("Vista simplificada para comunicación con enfermería y residentes. "
               "Actualiza los datos en las pestañas anteriores primero.")

    mod_enf, filtro_enf, _ = combinar_recomendaciones(escenarios)
    filtro_enf = st.session_state.get("ui_filtro", filtro_enf)
    qp_e, qp_h_e, qe_e, qr_pre_e, qr_post_e, qd_e, ff_e = flows_and_ff(
        qb, hto, dosis_mlkg, peso, uf, mod_enf or "CVVHDF")
    ac_tipo_enf = st.session_state.get("anticoagulacion_tipo", "—")
    cit_ml_enf = st.session_state.get("rca_citrato_ml_h", 0)
    ca_ml_enf = st.session_state.get("rca_calcio_ml_h", 0)
    hnf_ui_enf = st.session_state.get("hnf_ui_h", int(peso * 5))
    rca_t_enf = st.session_state.get("rca_targets", {})

    orden_md = f"""
---
## 🫀 ORDEN DE TERAPIA EXTRACORPÓREA — TRRC360

**Fecha/hora:** {datetime.now().strftime('%d/%m/%Y %H:%M')}  
**Modalidad:** `{mod_enf or '—'}`  
**Filtro:** `{filtro_enf or '—'}`  
**FF estimada:** `{ff_e:.1%}` (objetivo <25%)
...
| Parámetro | Valor |
|-----------|-------|
| Flujo sanguíneo QB | **{qb} mL/min** |
| Reposición PRE-filtro | **{qr_pre_e} mL/hr** |
| Reposición POST-filtro | **{qr_post_e} mL/hr** |
| Dialisato Qd | **{int(qd_e)} mL/hr** |
| UF neta | **{uf} mL/hr** |
| Efluente total | **{int(qe_e)} mL/hr** |

---

### 💊 ANTICOAGULACIÓN — {ac_tipo_enf}

{'**Citrato trisódico 4%:** ' + f'{cit_ml_enf:.0f} mL/hr por línea PRE-filtro' if ac_tipo_enf == 'RCA' else f'**HNF:** {hnf_ui_enf:.0f} UI/hr (ajustar a aPTT)'}  
{'**Ca-Gluconato 10%:** ' + f'{ca_ml_enf:.0f} mL/hr por línea sistémica POST-filtro' if ac_tipo_enf == 'RCA' else ''}

---

### 🔬 MONITOREO
{'| Parámetro | Frecuencia | Objetivo |' + chr(10) + '|-----------|-----------|---------|' + chr(10) + f'| iCa post-filtro | c/30min inicio, luego c/4-6h | **0.25–0.40 mmol/L** |' + chr(10) + f'| iCa sistémico | c/30min inicio, luego c/4-6h | **1.0–1.2 mmol/L** |' + chr(10) + '| BMP (Na, K, HCO₃⁻, AG) | Cada 4–6 hrs | Ver metas |' + chr(10) + '| Ca total + Ca iónico | Cada 12–24 hrs | Ratio <2.5 |' if ac_tipo_enf == 'RCA' else '| Parámetro | Frecuencia |' + chr(10) + '|-----------|-----------|' + chr(10) + '| aPTT | Cada 4–6 hrs |' + chr(10) + '| BMP | Cada 4–6 hrs |'}

---

### ⚠️ ALERTAS INMEDIATAS

- **FF >25%:** Reducir UF o reposición post, aumentar predilución. Avisar médico.
- **Presiones de transmembrana ↑:** Revisar coagulación del circuito.
{'- **iCa post-filtro <0.25:** Reducir citrato 10-20%. Avisar médico.' if ac_tipo_enf == 'RCA' else ''}
{'- **iCa sistémico <0.90:** Aumentar calcio 10-20%. Avisar médico.' if ac_tipo_enf == 'RCA' else ''}
{'- **Ratio Ca total/Ca iónico >2.5:** ACUMULACIÓN DE CITRATO. Avisar médico URGENTE.' if ac_tipo_enf == 'RCA' else ''}

---
*TRRC360 v{VERSION} — Dr. Tapia Nefrólogo — Uso académico*
"""
    st.markdown(orden_md)

    # Reposición de electrolitos en TRRC
    st.divider()
    st.markdown("### ⚗️ Guía de reposición de electrolitos en TRRC")
    k_actual = float(st.session_state.get("k_main", 4.0))
    ph_actual = float(st.session_state.get("ph_main", 7.35))
    hco3_actual = float(st.session_state.get("hco3_main", 20.0))

    st.markdown("**Potasio (K)**")
    if k_actual < 2.5:
        st.error(f"K = {k_actual:.1f} mEq/L — **Hipocalemia severa.** IV urgente + bolsas con KCl 40 mEq/L.")
    elif k_actual < 3.0:
        st.warning(f"K = {k_actual:.1f} mEq/L — Agregar **KCl 20–40 mEq/L** a las bolsas de reposición.")
    elif k_actual > 6.0:
        st.error(f"K = {k_actual:.1f} mEq/L — **Hipercalemia severa.** Bolsas con K 0 mEq/L. ↑ Qd.")
    elif k_actual > 5.5:
        st.warning(f"K = {k_actual:.1f} mEq/L — Bolsas con K 0–2 mEq/L. Monitoreo cada 2–4h.")
    else:
        st.success(f"K = {k_actual:.1f} mEq/L — Rango aceptable. Bolsas con K estándar (2–4 mEq/L).")

    st.markdown("**Bicarbonato / Buffer**")
    if ph_actual < 7.20 or hco3_actual < 15:
        st.error(f"pH = {ph_actual:.2f}, HCO₃⁻ = {hco3_actual:.1f} — **Acidosis severa.** "
                 "Usar soluciones con bicarbonato. Evitar lactato. ↑ Qd hasta 2–3 L/h.")
    elif ph_actual < 7.30 or hco3_actual < 18:
        st.warning(f"pH = {ph_actual:.2f}, HCO₃⁻ = {hco3_actual:.1f} — Acidosis moderada. "
                   "Preferir buffer de bicarbonato. Vigilar evolución.")
    elif ph_actual > 7.50 or hco3_actual > 30:
        st.warning(f"pH = {ph_actual:.2f}, HCO₃⁻ = {hco3_actual:.1f} — "
                   "Alcalosis. Si usando citrato, revisar tasa (citrato genera HCO₃⁻). "
                   "Reducir citrato si corresponde.")
    else:
        st.success(f"pH = {ph_actual:.2f}, HCO₃⁻ = {hco3_actual:.1f} — Equilibrio ácido-base aceptable.")

# ══════════════════════════════════════════════════════════════════════════════
# TAB 11: RESUMEN / PDF
# ══════════════════════════════════════════════════════════════════════════════
with tab_rx:
    st.subheader("Resumen de prescripción y exportación PDF")

    st.markdown("#### Identificación del paciente y médico")
    cU, _ = st.columns([3, 1])
    with cU:
    ...
    with r2c2:
        st.text_input("Expediente", key="rx_expediente",
    ...
    st.text_input("Sello / Notas", key="rx_sello",
                  value=st.session_state.get("rx_sello", ""))

    st.divider()
    mod_rx, filtro_rx, coment_rx = combinar_recomendaciones(escenarios)
    filtro_rx = st.session_state.get("ui_filtro", filtro_rx)
    qp_r, qp_h_r, qe_r, qr_pre_r, qr_post_r, qd_r, ff_r = flows_and_ff(
        qb, hto, dosis_mlkg, peso, uf, mod_rx or "CVVHDF")
    ff_txt_r = f"{ff_r:.2%}" if ff_r is not None else "—"

    st.write(f"**Escenarios:** {', '.join(escenarios) if escenarios else '—'}")
    st.write(f"**Modalidad:** {mod_rx or '—'} | **Filtro:** {filtro_rx or '—'} | **FF:** {ff_txt_r}")
    ca2, cb2, cc2, cd2 = st.columns(4)
    ca2.metric("Qb (mL/min)", qb)
    cb2.metric("Qe (mL/h)", int(qe_r))
    cc2.metric("UF (mL/h)", uf)
    cd2.metric("FF estimada", ff_txt_r)
    ce2, cf2, cg2 = st.columns(3)
    ce2.metric("Qr pre (mL/h)", qr_pre_r)
    cf2.metric("Qr post (mL/h)", qr_post_r)
    cg2.metric("Qd (mL/h)", int(qd_r))

    ac_rx = st.session_state.get("anticoagulacion_tipo", "—")
    if ac_rx == "RCA":
        st.info(f"**RCA** — Citrato: {st.session_state.get('rca_citrato_ml_h', 0):.0f} mL/h | "
                f"Ca-Gluconato: {st.session_state.get('rca_calcio_ml_h', 0):.0f} mL/h")
    elif ac_rx == "HNF":
        st.info(f"**HNF** — {st.session_state.get('hnf_ui_h', int(peso * 5)):.0f} UI/h")

    st.text_area("Comentarios para el PDF", key="rx_comentarios",
                 value=st.session_state.get("rx_comentarios", ""), height=100)

    st.markdown("### Opciones de PDF")
    st.write(f"PDF extendido (modo docente): **{'Sí' if st.session_state.get('pdf_extendido') else 'No'}**")
    st.caption("Cambia el modo docente en la barra lateral.")

    col_btn, _ = st.columns([1, 3])
    with col_btn:
        if st.button("📄 Generar PDF", key="btn_export_pdf"):
            try:
                fn = export_pdf()
