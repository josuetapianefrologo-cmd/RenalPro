...
/* ── ALERTAS — texto siempre legible ─────────────────────────────────────── */
div[data-testid="stAlert"] {
    border-radius: 10px !important;
}
/* Info */
div[data-testid="stAlert"][data-baseweb="notification"],
.stInfo, [data-testid="stAlert"].st-ae {
...
div[data-testid="stAlert"][kind="info"]   { background: #EFF6FF !important; border-left: 4px solid #2563EB !important; }
div[data-testid="stAlert"][kind="success"]{ background: #F0FDF4 !important; border-left: 4px solid #16A34A !important; }
div[data-testid="stAlert"][kind="warning"]{ background: #FFFBEB !important; border-left: 4px solid #D97706 !important; }
div[data-testid="stAlert"][kind="error"]  { background: #FEF2F2 !important; border-left: 4px solid #DC2626 !important; }
        ...
        if "nuevo" in aee_modo:
            ...
            with aee_n1:
                aee_nuevo_tipo = st.selectbox("AEE a iniciar",
                                              ...
                                              key="aee_n_tipo")
            with aee_n2:
                peso_aee = st.number_input("Peso (kg)", 10.0, 200.0, peso_anemia, 0.5, key="aee_n_peso")

            en_hd = "Hemodial" in an_contexto
            en_pd = "Peritoneal" in an_contexto

            if "Epoetina alfa" in aee_nuevo_tipo:
                dosis_low = 50 * peso_aee
                dosis_high = 100 * peso_aee
                frec_hd = "3 × semana (IV post-sesión de HD)"
                frec_nohd = "3 × semana SC (o 1× semana si usa esquema simplificado)"
                frec = frec_hd if en_hd else frec_nohd
                # Round to nearest available vial
...
| | |
|---|---|
| **Dosis recomendada** | **{dosis_low:.0f} – {dosis_high:.0f} UI** por aplicación |
| **Frecuencia** | {frec} |
| **Vial más cercano sugerido** | {vial_sug:,} UI |
| **Vía** | IV (HD) o SC (prediálisis / PD) |
| **Presentaciones** | 1,000 / 2,000 / 4,000 / 6,000 / 10,000 / 20,000 / 40,000 UI |
| **Primera evaluación de respuesta** | A las **4 semanas** |
| **Meta Hgb** | **10 – 11.5 g/dL** (no pasar de 13) |
                ...
                st.markdown(f"""
#### Darbepoetina alfa — Dosis inicial
...
- Presentaciones: 10 / 20 / 30 / 40 / 60 / 80 / 100 / 150 / 200 / 300 / 500 mcg
...
- Meta Hgb: **10 – 11.5 g/dL**
                ...
                st.markdown(f"""
#### Metoxi-PEG-epoetina beta (Mircera®) — Dosis inicial
| | |
|---|---|
...
| **Vía** | SC o IV |
| **Presentaciones** | 30 / 50 / 75 / 100 / 120 / 150 / 200 / 250 / 360 mcg/jeringa |
| **Ventaja** | Dosis mensual → mayor adherencia |
| **Primera evaluación** | A las **4 semanas** |
                """)
...
*Hiporrespuesta: Epoetina >300 UI/kg/sem o Darb >1.5 mcg/kg/sem sin alcanzar meta → investigar: déficit de hierro, infección, inflamación, hiperparatiroidismo, hemólisis.
        ...
        # ── MODO 3: CONVERSIÓN ────────────────────────────────────────────────
            ...
            st.caption("Conversión aproximada. Ajustar según respuesta clínica. Ref: KDIGO 2012, fichas técnicas.")

            cv1, cv2 = st.columns(2)
            with cv1:
                aee_from = st.selectbox("Convertir DESDE",
                                        ["Epoetina alfa (UI/semana)",
                                         "Darbepoetina alfa (mcg/semana)",
                                         "Darbepoetina alfa (mcg/2 semanas)"],
                                        key="aee_from")
                if "Epoetina" in aee_from:
                    dosis_from = st.number_input("Dosis semanal total de Epoetina (UI/sem)",
                                                 500, 200000, 6000, 500, key="cv_from_val")
                elif "semana)" in aee_from:
                    dosis_from = st.number_input("Dosis semanal de Darbepoetina (mcg/sem)",
                                                 5.0, 500.0, 20.0, 5.0, key="cv_from_val")
                else:
                    dosis_from = st.number_input("Dosis quincenal de Darbepoetina (mcg/2sem)",
            ...
            # Normalize to epoetina UI/week as common denominator
            if "Epoetina" in aee_from:
                epo_sem = dosis_from
            elif "semana)" in aee_from:
                epo_sem = dosis_from * 200  # 1 mcg darb = 200 IU epo
            else:
                epo_sem = (dosis_from / 2) * 200  # biweekly to weekly
            ...
            if "Darbepoetina" in aee_to and "semana)" in aee_to:
                ...
                if "Epoetina" in aee_from:
                    res_txt = f"Ya es Epoetina: **{dosis_from:,.0f} UI/semana**"
                    res_freq = "3 × semana"
                elif "semana)" in aee_from:
...
| Desde | Dosis | → | Hasta | Dosis equivalente | Frecuencia |
|-------|-------|---|-------|------------------|-----------|
| {aee_from} | {dosis_from:g} | **→** | {aee_to} | {res_txt} | {res_freq} |
            """)

            st.info("💡 Al cambiar de AEE: iniciar con la dosis equivalente calculada, "
                    "verificar Hgb a las **4 semanas** y ajustar ±25% según respuesta.")

            st.markdown("#### Tabla de referencia rápida — Epoetina ↔ Darbepoetina")
            st.markdown("""
| Epoetina (UI/sem) | Darbepoetina (mcg/sem) | Darbepoetina (mcg/2sem) | Mircera (mcg/mes) |
|------------------|----------------------|------------------------|------------------|
| <2,500 | 6.25 | 12.5 | 60 |
...
| ≥90,000 | 200 | 400 | 360 |
            """)
            st.caption("Ref: KDIGO 2012 Tabla 4. Mircera: datos de ficha técnica Roche. Ajustar siempre por respuesta clínica.")
