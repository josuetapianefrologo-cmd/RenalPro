# app.py — TRRC360 by Dr. Tapia (v1.19.4)
...
if not st.session_state.get("acepto_legal", False):
    st.header("Aviso legal (uso docente)")
    st.markdown("""
**TRRC360** es una herramienta **docente** para apoyar la prescripción y seguimiento de **TRRC/CRRT**.
No es un dispositivo médico ni sustituye el **juicio clínico** ni las guías locales.
El **uso y la aplicación** de la herramienta son responsabilidad del profesional que la utiliza.
""")
    if st.button("Acepto y comprendo", key="btn_acepto_legal"):
        st.session_state["acepto_legal"] = True
        st.rerun()
    st.stop()

with st.sidebar:
    st.subheader("Privacidad y almacenamiento local")
    st.caption("Los datos se guardan **solo** en este equipo, en un archivo JSON local.")
    acepta_priv = st.checkbox("He leído y acepto el aviso de privacidad", value=False, key="acepta_priv")
...
def export_pdf(filename: Optional[str] = None):
    ...
    c.setFont("Helvetica", 10); c.drawRightString(w - margin, y, datetime.now().strftime("%d/%m/%Y %H:%M"))
    y -= 28
    ...
    c.setFont("Helvetica", 11)
    comentarios_txt = st.session_state.get("rx_comentarios","") or "—"
    ...
    # Segunda página: Fundamento breve
    c.showPage(); y = h - 50
    c.setFont("Helvetica-Bold", 13); c.drawString(50, y, "Fundamento y Cálculos"); y -= 22
    c.setFont("Helvetica", 11)
    for linea in _fundamento_texto_resumen(*flows_current_context()):
...
def _fundamento_texto_resumen(qb, hto, qp, qp_h, qe, qr_pre, qr_post, qd, uf, ff_txt):
    return [
        ...
        "• FF = (Qr_post + UF) / (Qp·h + Qr_pre). Recomendado FF < 25%.",
        f"Contexto: Qb={_s_int(qb)}, Hto={hto:.2f}, Qp={_s_int(qp)}, Qp·h={_s_int(qp_h)}, Qe={_s_int(qe)}, Qr_pre={_s_int(qr_pre)}, Qr_post={_s_int(qr_post)}, Qd={_s_int(qd)}, UF={_s_int(uf)}, FF≈{ff_txt}."
    ]

# ================== BIBLIOGRAFÍA (dinámica por escenario) ==================
Ref = Dict[str, str]
BIBLIO: List[Ref] = [
    {"id":"dose_core_2024","yr":"2024","title":"Continuous Renal Replacement Therapy","where":"StatPearls/NCBI","url":"https://www.ncbi.nlm.nih.gov/books/NBK556028/","tags":"dosis,crrt,revision","blurb":"Dosis entregada 20–25 mL/kg/h; sin beneficio claro >25."},
    {"id":"rca_review_2023","yr":"2023","title":"Regional Citrate Anticoagulation in CRRT","where":"Review/PMC","url":"https://pmc.ncbi.nlm.nih.gov/articles/PMC10221969/","tags":"rca,anticoagulacion,crrt,revision","blurb":"RCA segura/efectiva; 1ª línea si disponible."},
...
with col_title:
    ...
    st.caption("Herramienta docente; no sustituye el juicio clínico.")

if st.button("🔁 Refrescar", key="btn_refresh"):
    try:
...
with st.sidebar:
    ...
    try:
        readme_data = open("README.md","r",encoding="utf-8").read()
    except Exception:
        readme_data = "README no disponible."
    try:
        license_data = open("LICENSE","r",encoding="utf-8").read()
    except Exception:
        license_data = "LICENSE no disponible."
    st.download_button("⬇️ README.md", data=readme_data, file_name="README.md", mime="text/markdown")
    st.download_button("⬇️ LICENSE (MIT)", data=license_data, file_name="LICENSE", mime="text/plain")
...
tab_main, tab_ktv, tab_balance, tab_anticoag, tab_trends, tab_fund, tab_rx, tab_refs = st.tabs([
    "Prescripción","Dosis por objetivos (Kt/V)","Balance dinámico","Anticoagulación","Tendencias",
    "Fundamento y Cálculos","Resumen / PDF","Referencias",
])
...
with tab_main:
    ...
    if ff_pct <= 25:
        st.success(f"FF ≈ {ff_pct:.1f}% — objetivo docente razonable (≤25%).")
    elif ff_pct <= 30:
    ...
    st.caption("Dosis objetivo 20–25 mL/kg/h como entregada; usar FF <25% para proteger el filtro (ver Referencias).")
...
with tab_anticoag:
    ...
    if ac == "Heparina no fraccionada (HNF)":
        ...
        rca2.metric("Reposición Ca²⁺ (mL/h)", f"{calcio_ml_h_ini:.0f}")
...
def evaluar_tendencia(v1, v3, tag):
    t = tag.lower().strip()
    R_NA=(135,145); R_K=(3.5,5.1); R_LAC=(0.5,2.2); R_NH4=(15,45)
    if t.startswith("na"):   return ("warn","Na fuera 135–145") if (v3<R_NA[0] or v3>R_NA[1]) else ("ok","Na 135–145")
    if t.startswith("k"):    return ("warn","K fuera 3.5–5.1")   if (v3<R_K[0]  or v3>R_K[1])  else ("ok","K 3.5–5.1")
    if t.startswith("lact"): return ("warn","Lactato >2.2")      if v3>R_LAC[1] else ("ok","Lactato ≤2.2")
    if t.startswith("amon"): return ("warn","Amonio >45")        if v3>R_NH4[1] else ("ok","Amonio ≤45")
    return ("ok","—")

def render_trend_chart(label: str, t1: float, t2: float, t3: float):
    try:
        import pandas as pd
        df = pd.DataFrame({"valor":[t1,t2,t3]}, index=["T1","T2","T3"])
        st.line_chart(df, height=140)
    except Exception:
        st.caption("Gráfica no disponible (pandas no instalado).")
...
with tab_trends:
    ...
    def fila_tendencia(etq, key, tag, vmin=0.0, vmax=1000.0, step=0.5, defaults=(0.0,0.0,0.0)):
        ...
        st.warning(msg) if level=="warn" else st.info(msg)
        render_trend_chart(etq, t1, t2, t3)
        st.markdown("---")
        return None
    DEF={"na":(140.0,137.0,135.0),"k":(4.0,4.8,6.2),"lact":(1.2,2.6,3.1),"nh4":(60.0,120.0,180.0),"ure":(130.0,160.0,210.0),"crn":(3.2,4.0,4.5)}
...
with tab_fund:
    ...
    mostrar_ext = st.checkbox("Vista extendida", value=False, key="mostrar_fund_extendido")
    ...
    st.latex(r"Q_d = \max\left(Q_e - (Q_{r,pre} + Q_{r,post} + UF),\ 0\right)")
    st.latex(r"FF = \dfrac{Q_{r,post} + UF}{Q_{p,h} + Q_{r,pre}} \quad (<25\%)")
    qb, hto_v, qp, qp_h, qe, qr_pre, qr_post, qd, uf_v, ff_txt = flows_current_context()
    c1,c2,c3 = st.columns(3)
    with c1:
        st.write(f"**Qb** = {int(qb)}"); st.write(f"**Hto** = {hto_v:.2f}"); st.write(f"**Qp** = {int(qp)}")
    with c2:
        st.write(f"**Qp·h** = {int(qp_h)}"); st.write(f"**Qe** = {int(qe)}"); st.write(f"**UF** = {int(uf_v)}")
    with c3:
        st.write(f"**Qr_pre** = {int(qr_pre)}"); st.write(f"**Qr_post** = {int(qr_post)}"); st.write(f"**Qd** = {int(qd)}"); st.write(f"**FF** ≈ {ff_txt}")
    if mostrar_ext:
        for linea in _fundamento_texto_resumen(qb, hto_v, qp, qp_h, qe, qr_pre, qr_post, qd, uf_v, ff_txt):
            st.write(linea)
...
# ====== README y LICENSE embebidos ======
with st.expander("📘 Manual de uso de TRRC360 (README)", expanded=False):
    st.markdown("""
1) Instalar (si local): `pip install streamlit reportlab pandas`  
2) Ejecutar: `streamlit run app.py`  
3) Flujos, FF y modalidad se calculan con base en escenario + parámetros.  
**Docente**: no sustituye juicio clínico; datos se guardan localmente si aceptas privacidad.
""")
