# Writing the updated Streamlit app (app.py) with requested changes:
# - Removed password gate entirely
# - Added privacy notice gate at startup (must accept to continue)
# - Kept sidebar (modo docente, parámetros básicos, estados clínicos) unchanged in spirit
# - Left tabs: Prescripción, Kt/V, Anticoagulación extendida, Referencias unchanged
# - Removed "Tendencias de laboratorio" tab entirely
# - Kept "Fundamento y Cálculos" content but removed the st.info/st.caption banner that was noisy
# - Kept PDF export and bibliography logic

app_code = r'''
# app.py — TRRC360 by Dr. Tapia (v1.9.0)
...
# ================== PRIVACY GATE (reemplaza password) ==================
...
# ---------- Sidebar inputs (sin cambios conceptuales) ----------
with st.sidebar:
    # ===== Switch global de modo docente =====
    st.header("Modo")
    ...
    qb   = st.number_input("Qb (mL/min)", 80, 300, st.session_state.get("sb_qb", 200), 10, key="sb_qb")
    uf   = st.number_input("UF (mL/h)", 0, 2000, st.session_state.get("sb_uf", 100), 10, key="sb_uf")
    dosis_mlkg = st.slider("Dosis objetivo (mL/kg/h)", 10, 45, st.session_state.get("sb_dosis", 30), key="sb_dosis")
...
def export_pdf():
    """Genera PDF con opción de fundamento RESUMEN o EXTENDIDO + Referencias."""
    s = st.session_state
...
with tab_main:
    ...
    if cond_qd_alto:
        ...
        if aplicar_auto:
            ...
            st.caption("Nota: ajusta soluciones a **K 0–2 mmol/L** hasta K <5.5; confirma electrolitos cada 2–4 h.")
...
# ---------- Fundamento y Cálculos (sin banner st.info/caption molesto) ----------
with tab_fund:
    st.subheader("Fundamento y Cálculos")
    mostrar_ext = bool(st.session_state.get("mostrar_fund_extendido", False))

    # (Se quita cualquier st.info/st.caption de estado extendido aquí)
