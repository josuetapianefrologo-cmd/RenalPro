        ...
        if mp_token and uid:
            if st.button("🔗 Pagar con Mercado Pago — Tarjeta / OXXO / Transferencia",
                ...
                with st.spinner("Generando tu link de pago..."):
                    ...
                    # Fallback: llamar directo con secrets
                    if not link and mp_token:
                        try:
                            import mercadopago
                            sdk = mercadopago.SDK(mp_token)
                            app_url = "https://trrc360.streamlit.app"
                            try:
                                app_url = st.secrets.get("APP_URL", app_url)
                            except Exception:
                                pass
                            pref = sdk.preference().create({
                                "items": [{"title": "TRRC360 Pro — 1 mes",
                                           "quantity": 1, "unit_price": 99.0,
                                           "currency_id": "MXN"}],
                                "external_reference": f"user_{uid}_{st.session_state.get('sess_user','')}",
                            ...
                            link = pref.get("response", {}).get("init_point")
                        except Exception:
                            link = None
...
<div style="text-align:center;padding:16px;">
  ...
  <p style="color:#64748B;font-size:12px;margin-top:8px;">
    Acepta: tarjeta, OXXO, transferencia bancaria, efectivo
                    ...
                    st.info("✅ Después de pagar, tu cuenta se activa automáticamente en segundos. "
                            "Si no ves el cambio en 5 minutos, cierra sesión y vuelve a entrar.")
                else:
                    st.error("No se pudo generar el link de pago. Usa la opción manual abajo.")
        elif not mp_token:
            st.warning("🔧 Mercado Pago no configurado aún. Usa la opción de transferencia manual.")
        elif not uid:
            st.warning("⚠️ Crea una cuenta o inicia sesión para generar tu link de pago.")
