...
<div style="text-align:center;padding:36px 0 16px 0;">
  ...
  <div style="color:rgba(255,255,255,0.55);font-size:12px;margin-top:4px;">
    ...
    with col:
        ...
        with login_tab:
            u = st.text_input("Usuario", placeholder="tu_usuario", key="li_u", label_visibility="collapsed")
            p = st.text_input("Contraseña", type="password", placeholder="contraseña", key="li_p", label_visibility="collapsed")
...
# Banner de estado + cerrar sesión
_status_banner()
with st.sidebar:
    st.markdown("---")
    if st.button("🚪 Cerrar sesión", key="btn_logout", use_container_width=True):
        for k in ["logged_in", "sess_user", "sess_rol", "sess_nombre", "consent_ok"]:
...
# ─── TABS ─────────────────────────────────────────────────────────────────────
(tab_main, tab_scores, tab_citrato, tab_na_trrc, tab_hd, tab_tpe,
 tab_ktv, tab_balance, tab_anticoag, tab_fund,
 tab_resumen, tab_premium, tab_admin, tab_refs) = st.tabs([
    "🩺 Prescripción",
    "📊 Scores / Candidatura",
    "🧪 Citrato RCA",
    "🧂 Sodio TRRC",
    ...
    "📋 Resumen / Impresión",
    "💳 Premium",
    "🛡️ Admin" if _rol() == "admin" else "👤 Mi cuenta",
    "📖 Referencias",
])
