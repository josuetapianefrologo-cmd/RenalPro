...
# ── Carga config desde clinical_data (con fallback defensivo) ─────────────────
try:
    from clinical_data import get_dashboard_tr_config as _get_dash_cfg
    from clinical_data import tac_meta_por_dpt as _cd_tac_meta
    _DASH_CFG_OK = True
except ImportError:
    _DASH_CFG_OK = False
    _get_dash_cfg = None
...
# Defaults internos (último recurso si clinical_data no está disponible)
_DASH_DEFAULTS = {
    "tr_note_types": [
...
def _dash_cfg() -> Dict:
    """Devuelve config del dashboard (de JSON via clinical_data o defaults)."""
    if _DASH_CFG_OK and _get_dash_cfg:
        try:
            return _get_dash_cfg()
        except Exception:
            pass
    return _DASH_DEFAULTS


def _tr_note_types_tuple() -> tuple:
    """Devuelve tipos de nota considerados 'evidencia de trasplante' como tuple para SQL IN."""
    cfg = _dash_cfg()
    types = cfg.get("tr_note_types") or _DASH_DEFAULTS["tr_note_types"]
    return tuple(types)


# Constante para retrocompatibilidad (usada en queries que la importan directamente)
TR_NOTE_TYPES = _tr_note_types_tuple()
...
def get_cohorte_tr(user_id: int) -> List[Dict]:
    ...
    try:
        cur = conn.cursor()
        _tr_types = _tr_note_types_tuple()
...
def _build_cohorte_row(patient_id: int) -> Optional[Dict]:
    ...
    try:
        ...
        cur.execute("""
            ...
            ORDER BY created_at DESC
            LIMIT 1
        """, (patient_id, _tr_note_types_tuple()))
        ...
        # ── ALERTAS automáticas (umbrales y mensajes del JSON) ──────────
        cfg = _dash_cfg()
        umbrales_cr = cfg.get("umbrales_cr") or _DASH_DEFAULTS["umbrales_cr"]
        umbrales_sn = cfg.get("umbrales_sin_nota") or _DASH_DEFAULTS["umbrales_sin_nota"]
        msgs = cfg.get("alertas_mensajes") or _DASH_DEFAULTS["alertas_mensajes"]
        tac_margen = cfg.get("tac_alto_margen_extra", 3)

        cr_critico_th = float(umbrales_cr.get("critico_delta_pct", 25))
        cr_alerta_th = float(umbrales_cr.get("alerta_delta_pct", 15))
        sn_primer6m = int(umbrales_sn.get("primer_6m_dias", 30))
        sn_despues = int(umbrales_sn.get("despues_6m_dias", 60))

        alertas = []

        def _fmt(template: str, **kwargs) -> str:
            """Formato seguro de mensaje (si falla, devuelve template crudo)."""
            try:
        ...
        if delta_cr is not None:
            try:
                ...
                if d_val > cr_critico_th:
                    alertas.append(_fmt(msgs.get("cr_critico", ""), delta=d_val))
                elif d_val > cr_alerta_th:
                    alertas.append(_fmt(msgs.get("cr_alerta", ""), delta=d_val))
            except Exception:
                pass

        # 2. Tac C0 fuera de meta según DPT (usa tac_meta_por_dpt del JSON)
        tac_c0 = datos.get("tac_c0")
        if tac_c0 is not None and dpt is not None:
            try:
                tac_val = float(tac_c0)
                dpt_val = int(dpt)
                # Usar función del loader si está disponible
                if _DASH_CFG_OK and _cd_tac_meta:
                    meta_min, meta_max, _ = _cd_tac_meta(dpt_val)
                else:
                    # Fallback hardcoded
                    if dpt_val <= 30:
                        meta_min, meta_max = 8.0, 12.0
                    elif dpt_val <= 180:
                        meta_min, meta_max = 6.0, 10.0
                    else:
                        meta_min, meta_max = 5.0, 8.0
                if tac_val < meta_min:
                    alertas.append(_fmt(msgs.get("tac_bajo", ""),
                                        tac=tac_val, min=meta_min, max=meta_max))
                elif tac_val > meta_max + float(tac_margen):
                    alertas.append(_fmt(msgs.get("tac_alto", ""),
                                        tac=tac_val, min=meta_min, max=meta_max))
            except Exception:
                pass
        ...
        if dias_ult is not None:
            if dias_ult > sn_despues:
                alertas.append(_fmt(msgs.get("sin_nota_largo", ""), dias=dias_ult))
            elif dias_ult > sn_primer6m and dpt and dpt < 180:
                alertas.append(_fmt(msgs.get("sin_nota_primer_6m", ""), dias=dias_ult))

        # 4. Sin nota Post-TR pero marcado manual
        if pac.get("es_trasplantado") and not ult_nota:
            alertas.append(msgs.get("marcado_sin_nota", "🔵 Marcado como TR sin notas"))
...
def estadisticas_cohorte_tr(user_id: int) -> Dict:
    ...
    # Por tiempo post-TR — usa categorías del JSON
    cfg = _dash_cfg()
    cats = cfg.get("tiempo_post_tr_categorias") or [
    ...
    por_tiempo = {c["label"]: 0 for c in cats}
    por_tiempo["Sin fecha"] = 0
    for c in cohorte:
        dpt = c.get("dpt")
        if dpt is None:
        ...
        for cat in cats:
            try:
                if int(cat.get("min", 0)) <= dpt <= int(cat.get("max", 99999)):
                    por_tiempo[cat["label"]] += 1
                    break
